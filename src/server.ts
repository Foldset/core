...
import type { ConfigStore, ProcessRequestResult } from "./types";
...
export interface FoldsetX402HTTPServer extends x402HTTPResourceServer {
  processHTTPRequest(context: HTTPRequestContext, paywallConfig?: PaywallConfig): Promise<ProcessRequestResult>;
}
...
const processHTTPRequestWithRestriction: FoldsetX402HTTPServer["processHTTPRequest"] = async function (
  ...
  if (result.type === "payment-error") {
    ...
    return { ...result, restriction: routeConfig?.restriction } as ProcessRequestResult;
  }
  return result as ProcessRequestResult;
};
...
export class HttpServerManager {
  ...
  async get(): Promise<FoldsetX402HTTPServer | null> {
    ...
    const contentRoutes = buildRoutesConfig(restrictions, paymentMethods, hostConfig.legalUrl);
    const mcpRoutes = hostConfig.mcpEndpoint
      ? buildMcpRoutesConfig(restrictions, paymentMethods, hostConfig.mcpEndpoint, hostConfig.legalUrl)
      : {};
